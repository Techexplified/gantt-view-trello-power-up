...
export default function GanttDashboard({ initialBoardId, onLogout }) {
  ...
  return (
    <div style={styles.layout}>
      ...
      {/* ── Card Detail Modal ── */}
      {selectedCard && (
        <CardModal card={selectedCard} lists={lists} onClose={() => setSelectedCard(null)} />
      )}
    </div>
  );
}
...
function EmptyState() {
  return (
    <div style={styles.centered}>
      ...
      <p style={styles.emptyText}>Choose a board from the left sidebar to see its calendar view.</p>
    </div>
  );
}
...
function CardModal({ card, lists, onClose }) {
  ...
  const due = card.due ? new Date(card.due).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : null;
  const start = card.start ? new Date(card.start).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : null;
    ...
    <div style={styles.modalOverlay} onClick={onClose}>
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={styles.row}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {card.labels.map((lbl, i) => (
                  <span key={i} style={{ ...styles.labelChip, background: lbl.color }}>{lbl.name || lbl.color}</span>
                ))}
              </div>
            </div>
          )}
        ...
        <div style={styles.modalFooter}>
          <a href={card.url} target="_blank" rel="noreferrer" style={styles.openBtn}>
            Open in Trello ↗
          </a>
        </div>
      </div>
...
const styles = {
  ...
  modalTitle: { color: '#e6edf3', fontSize: 16, fontWeight: 700, margin: 0, flex: 1, lineHeight: 1.4 },
  closeBtn: { background: 'none', border: 'none', color: '#8b949e', fontSize: 18, cursor: 'pointer', marginLeft: 12, padding: 0 },
  ...
  labelChip: { fontSize: 11, padding: '2px 8px', borderRadius: 4, color: '#fff', fontWeight: 500 },
  modalFooter: { padding: '12px 20px 20px', display: 'flex', justifyContent: 'flex-end' },
  openBtn: {
    background: 'rgba(0,121,191,0.2)', border: '1px solid rgba(0,121,191,0.5)',
    color: '#58a6ff', borderRadius: 8, padding: '8px 16px', fontSize: 13,
